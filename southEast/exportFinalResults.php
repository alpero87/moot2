...
*/
    ...
    if (mysql_num_rows($result) > 0)
    {
        ...
        for($indx = 0; $indx < mysql_num_rows($result); $indx++)
        {
            ...
            @mysql_data_seek($result, $indx); $data = mysql_fetch_assoc($result);
            $ranks[] = array('team_ID' => $data['team_ID'], 'win' => $data['win'], 'tie' => $data['tie'], 'loss' => $data['loss'], 'prelimBallots' => $data['prelimBallots'], 'total' => $data['total'], 'strength' => $data['strength']);
        }
...
//	print_r($ranks);
		...
		{
		    $sort['prelimBallots'][$k] = $v['prelimBallots'];
		    $sort['strength'][$k] = $v['strength'];
		    $sort['total'][$k] = $v['total'];
		}
		# sort by event_type desc and then title asc
		array_multisort($sort['prelimBallots'], SORT_DESC, $sort['strength'], SORT_DESC, $sort['total'], SORT_DESC, $ranks);

//	print_r($ranks); exit;


		for($indx0 = 0; $indx0 < sizeof($ranks); $indx0++)
		{
